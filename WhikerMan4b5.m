function varargout = WhikerMan4b5(varargin)
...
% Last Modified by GUIDE v2.5 26-Oct-2016 15:20:24
...
set(handles.edit_snout_sigma,'String',9)
...
% new in v4b4:
set(handles.edit_snout_outliers,'String',6)
...
function [handles] = fit_snakes(handles)
...
testframes = round(linspace(trframe(1),trframe(end),100));
...
clear fridx fr w r3 r2 z0 np0 np1 wmod
...
for w = 1:handles.Nwhiskers
    ...
    disp(sprintf('Best solution for whisker %d: E=%.1f ',w,Emin(w))),
    if Emin(w)<handles.whisker(w).energy_threshold
        goodsolution(w) = true;
        disp('...good')
    else
        disp('...bad')
