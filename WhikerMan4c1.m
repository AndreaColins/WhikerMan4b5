function varargout = WhikerMan4c1(varargin)
...
% Last Modified by GUIDE v2.5 02-Nov-2017 11:31:54
...
handles.colours = {'b.-','g.-','r.-','c.-','y.-','m.-'};
handles.lines = {'b-','g-','r-','c-','y-','m-'};
handles.lines_dotted = {'b:','g:','r:','c:','y:','m:'};
handles.points = {'b.','g.','r.','c.','y.','m.'};

handles.mastervideo_selected = false;   % mastervideo is the horizontal view

% new in v4b4:
set(handles.edit_snout_outliers,'String',1)
...
handles.dir.h ='C:\Users\mqbpwac5\Desktop\VideosSpikeSorting\Glu32_21092017H\';
handles.dir.v ='C:\Users\mqbpwac5\Desktop\VideosSpikeSorting\Glu32_21092017V\';

%handles.dir.h =  'C:\Users\mjcssrp\Desktop\WhikerMan4b5\ExampleH\';
%handles.dir.v =  'C:\Users\mjcssrp\Desktop\WhikerMan4b5\ExampleV\';
...
if handles.mastervideo_selected
    ...
    clear hsize sigma
    % meanframe
    meanframe = init_subtract_image_mean(get(handles.radiobutton_subtract_image_mean,'value'),handles.roi,handles.video);
...
%     end
    ...
    if ~isfield(handles,'folliclemask') || (mod(handles.currentframe.h-handles.video.h.startframe ,2)==0)
    ...
    handles.folliclemask{handles.currentframe.h,2} = folliclemask{2};
        ...
        if handles.currentframe.h==1
       
            [folliclemask{1}]=handles.folliclemask{3488,1};
        [folliclemask{2}]=handles.folliclemask{3488,2};
        
        handles.folliclemask{handles.currentframe.h,1} = folliclemask{1};
        handles.folliclemask{handles.currentframe.h,2} = folliclemask{2};
        else
       
        [folliclemask{1}]=handles.folliclemask{handles.currentframe.h-1,1};
        [folliclemask{2}]=handles.folliclemask{handles.currentframe.h-1,2};
...
%     clear folliclemask
    ...
    if any(firstframe)
        ...
        if strmatch(trfile(end-2:end),'tr4')
...
%         plot_frame(handles.frame,handles.video,handles.meanframe,titles,handles)
        ...
        for attempt = 1:nattempts
            ...
            [z,Emin,exitflag,output] = fminunc(@(z) Bimfun_wm4(z, r3, np0, np1, im.h, im.v, handles.dt, ...
                ...
                optimset('GradObj','On','TolX',.05,'Display','off'));
...
%     handles.frame = load_frame_2views(handles.video,frameidx,handles.roi);%,get(handles.checkbox_rotatevview,'value'));
    ...
    %             titlestring = sprintf('%d (%.1f)',frameidx,handles.Emin(frameidx));
    %         end
    %     end
%     set(handles.currentframe_display,'string',titlestring)
%     clear titlestring
    ...
    %     cla
    %     xlim([1 max([handles.video.width.h,handles.video.width.v])])
    %     ylim([1 max([handles.video.height.h,handles.video.height.v])])
    %     set(gca,'ydir','reverse')
    %     axis image
    %     hold on
    %     imagesc(double(handles.frame.h(:,:,1)) - handles.meanframe.h(:,:,1));
...
%     plot_frames(handles.frame,handles.video,handles.meanframe,titles,haxes)   
    ...
    if isfield(handles.whisker,'r3all')
        ...
        cla, hold on
        if get(handles.radiobutton_plot_snout_contour,'value') && (size(handles.folliclemask,1)>=frameidx)
            frame_is_tracked = ~isempty(handles.folliclemask{frameidx,1});
            if isfield(handles,'folliclemask') && frame_is_tracked
                axes(haxes.h)
...
if change_whisker
    ...
    update_current_whisker_display(w, handles)
    titlestring = sprintf('H frame %d',frameidx);
    if isfield(handles.whisker,'tracked')
        if handles.whisker(w).tracked(frameidx) && isfield(handles.whisker,'Emin')
            titlestring = [titlestring sprintf('(%.1f)',handles.whisker(w).Emin(frameidx))];
        end
    end
    set(handles.currentframe_display,'string',titlestring)
    clear titlestring
...
while (batch_fid && ~stopbatch)
    ...
    handles.fname.h =datfiles{1}; % extend this to get both h and v file from the .bat
    handles.fname.v =datfiles{2};
    fprintf(report_fid,' Opening video %s\n',handles.fname.h)    % ditto
    % stuff
    % choose the file selected in the autoinitialisation menu
%     ff = dir(['*.tr4']); % correct to new tr4 format %corrected
%     if length(ff)>0
%         nfiles=size(ff,1);
...
%         end
%         clear i nfiles
%         [~,idx] = min(datenum); %%%%the oldest one
%         fn = ff(idx).name;
%        fn='Glu32_210917a_20170921_140058.tr4';
...
%     handles.roi = roi;
    ...
    if get(handles.radiobutton_subtract_image_mean,'Value')
        whisker(1,1:handles.Nwhiskers).energy_threshold = handles.default_energy_threshold_subtract_image_mean;
    else
...
%     trpmtrs.miumax = str2double(get(handles.edit_miumax,'String'));
    trpmtrs.subtract_image_mean = get(handles.radiobutton_subtract_image_mean,'value');
    handles.trpmtrs = trpmtrs;
...
if get(handles.radiobutton_subtract_image_mean,'Value')
    handles.whisker(handles.current_whisker).energy_threshold = handles.default_energy_threshold_subtract_image_mean;
else
...
for w = 1:handles.Nwhiskers
    wmod = rem(w,handles.Nwhiskers) + handles.Nwhiskers*(w==handles.Nwhiskers); % to avoid running out of colours
    handles.whisker(w).selected
    handles.whisker(w).tracked(frameidx)
    if handles.whisker(w).selected && handles.whisker(w).tracked(frameidx)
        dt = 0.05;
        t = 0:dt:1;
        r3 = squeeze(handles.whisker(w).r3all(frameidx,:,:));
